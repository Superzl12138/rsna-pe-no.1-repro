...
LOG_DIR="${REPRO_OUTPUT_ROOT:-/data/zilizhu/PE/repro_outputs}/trainval/seresnext101"
mkdir -p "$LOG_DIR"
bash "$ROOT_DIR/prepare_output_links.sh" "$SCRIPT_DIR" "$LOG_DIR" weights features0

START_STEP="${SERESNEXT101_START_STEP:-train}"
...
should_run() {
  ...
  case "$step" in
    ...
    *) echo "[ERROR] Unknown seresnext101 step: $step"; exit 1 ;;
  esac
  case "$START_STEP" in
    train) start_order=1 ;;
    valid) start_order=2 ;;
    ...
    *) echo "[ERROR] Unknown SERESNEXT101_START_STEP: $START_STEP"; exit 1 ;;
  esac
