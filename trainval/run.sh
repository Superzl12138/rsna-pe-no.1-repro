...
if should_run_stage process_input && [[ "${SKIP_PROCESS_INPUT:-0}" != "1" ]]; then
  cd process_input
  sh run.sh
  cd ..
elif should_run_stage process_input; then
  echo "[INFO] 跳过 process_input，直接复用现有 pickle。"
fi
if should_run_stage lung_localization && [[ "${SKIP_LUNG_LOCALIZATION:-0}" != "1" ]]; then
  cd lung_localization/split2
  sh run.sh
  cd ..
  cd ..
elif should_run_stage lung_localization; then
